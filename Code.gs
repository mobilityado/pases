...
function createPdf_(tipo,r) {
  const folio=val_(r,'FOLIO');
  const logo='data:image/jpeg;base64,'+Utilities.base64Encode(logoBlob_().getBytes());
  const isAc=tipo==='ACLARACION';
  const title=isAc?'PASE DE ACLARACIÓN':'PASE DE NO ADEUDO';
  ...
  const trs=rows.map(x=>'<tr><th>'+htmlEsc_(x[0])+'</th><td>'+htmlEsc_(x[1])+'</td><th>'+htmlEsc_(x[2])+'</th><td>'+htmlEsc_(x[3])+'</td></tr>').join('');
  const firmas=isAc ?
    '<div class="firmas"><div>FIRMA Y SELLO<br>TACOGRAFÍA</div><div>NOMBRE Y FIRMA<br>CONDUCTOR</div><div>NOMBRE, FIRMA Y SELLO<br>AUTORIZADO</div></div>' :
    '<div class="firma1">CLAVE Y NOMBRE COMPLETO DEL RECAUDADOR QUE AUTORIZÓ EL NO ADEUDO</div>';

  const html='<!doctype html><html><head><meta charset="utf-8"><style>'+
  '@page{size:20cm 12cm landscape;margin:0}*{box-sizing:border-box}body{margin:0;font-family:Arial,sans-serif;color:#202124}'+
  '.pass{width:20cm;height:12cm;border:5px solid #552583;padding:.28cm;background:white;position:relative;overflow:hidden}'+
  '.top{height:2.55cm;display:flex;align-items:center;justify-content:center;border-bottom:3px solid #552583;position:relative}'+
  '.logo{height:2.35cm;max-width:7.3cm;object-fit:contain}.folio{position:absolute;right:.15cm;bottom:.18cm;background:#ed1c24;color:#fff;padding:.13cm .25cm;font-weight:800;border-radius:4px;font-size:10pt}'+
  '.title{background:#552583;color:#fff;text-align:center;font-weight:800;font-size:13pt;padding:.13cm;letter-spacing:.3px}'+
  'table{width:100%;border-collapse:collapse;margin-top:.16cm;table-layout:fixed}th,td{border:1.4px solid #777;padding:.11cm .12cm;font-size:8.3pt;height:.56cm}th{background:#f1eaf5;color:#552583;text-align:left;width:18%;font-weight:800}td{width:32%;font-weight:600}'+
  '.firmas{position:absolute;left:.45cm;right:.45cm;bottom:.35cm;display:flex;gap:.45cm}.firmas div{flex:1;border-top:1.5px solid #333;text-align:center;padding-top:.08cm;font-size:6.5pt;font-weight:700}'+
  '.firma1{position:absolute;left:5cm;right:5cm;bottom:.4cm;border-top:1.5px solid #333;text-align:center;padding-top:.08cm;font-size:6.7pt;font-weight:700;color:#552583}'+
  '</style></head><body><div class="pass"><div class="top"><img class="logo" src="'+logo+'"><div class="folio">FOLIO '+htmlEsc_(folio)+'</div></div>'+
  '<div class="title">'+title+'</div><table>'+trs+'</table>'+firmas+'</div></body></html>';

  // Apps Script convierte el HTML renderizado a PDF conservando el diseño.
  return Utilities.newBlob(html,'text/html',folio+'.html').getAs(MimeType.PDF).setName(folio+'.pdf');
}
